...
5. 当前 CJS 产物使用 `.cjs.js` 后缀，在 `type: module` 包中无法作为标准 CommonJS 入口工作。
...
> 2026-08-23 决策：`1.13.2` 采用候选 B。实际 tarball 已在禁用 lifecycle
> scripts 的环境中通过 Vue 2.6、Vue 2.7、Vue 3 的 ESM/CJS 显式入口验证；
> 启用 scripts 时，三个版本的 legacy root 入口也通过验证。候选 A/C 暂缓，
> 只有多入口方案不能满足真实使用时才继续投入。

1. 当前采用候选 B，先提供稳定显式入口，并保留根入口兼容。
2. 候选 A/C 不进入本轮实现；只有候选 B 被真实用例证明不足时才继续验证。
3. 发布包内不得把某次本地 `vue-demi-switch` 的残留结果当作隐式默认值。
